...
const bfs = (adjList, start, end) => {
    ...
    const visited = {}
    while(queue.length){
        const [parNode, ind] = queue.shift();
        if(parNode === end) return ind;
           
        for(let node of adjList[parNode]){
            if(!visited[node]){
                queue.push([node, ind+1]);
                visited[node] = 1;
            }
        }
    }
...
const wordLadder = (dictionary, start, end) => {
    if(!dictionary.includes(end)) return 0;
...
console.log(wordLadder(["hot", "dot", "dog", "lot", "log", "cog"], "hit", "cog")) //4

console.log(wordLadder(["hot", "dot", "dog", "lot", "log"], "hit", "cog")) //0

console.log(wordLadder([ "dot", "dog", "lot", "log", "cog"], "hot", "cog")) //3
